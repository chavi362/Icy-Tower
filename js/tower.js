...
function drawBlocks() {
    blocks.forEach(function (item) {
        if (item.typeOfImg == 1)
            ...
            context.drawImage(secondBlockImg, item.x, item.y, item.width, item.height);
        else
...
function initBlocks() {
    blocks = [];
    let width = randomMinMax(minmumBlockWidth, canvas.width * 0.09 + minmumBlockWidth);
    let height = 6;
    ...
    for (let i = 1; i < 5; i++) {
        width = randomMinMax(minmumBlockWidth, canvas.width * 0.09 + minmumBlockWidth);
        x = (randomMinMax(horizontalDistance, horizontalDistance * 1.2) + blocks[i - 1].x) % (canvas.width - width);
