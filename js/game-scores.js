...
function updateScores() {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (!highScore)
            arrOfObjects.push({ "name": currentUser.firstName, "userName": currentUser.userName, "score": 0 })
        else
            arrOfObjects.push(highScore);
    }
   ...
   console.log(currentUser.secondScore) ;
    for (let i = 0; i < 3; i++) {
        ...
        if (!foundSecond && currentUser.secondScore > scoreToPut.score) {
            ...
            foundSecond = true;
            if (arrOfObjects[j].userName == currentUser.userName && arrOfObjects[j].userName == currentUser.secondScore) {
                j++;
                i++;
                continue;
            }
            if (currentUser.secondScore > arrOfObjects[j]["score"]) {
                setArr.push({ "name": currentUser.firstName, "userName": currentUser.userName, "score": currentUser.secondScore })
            }
            else {
            ...
            if (arrOfObjects[j].userName == scoreToPut.userName && arrOfObjects[j].userName == scoreToPut.score) {
                j++;
                i++;
                continue;
            }
            if (scoreToPut.score > arrOfObjects[j]["score"]) {
                setArr.push({ "name": scoreToPut.firstName, "userName": scoreToPut.userName, "score": scoreToPut.score })
    ...
    function updateScoreInHtml() {
        ...
        firstHighScore.innerHTML = "1-" + arrOfObjects[0]["name"] + " " + arrOfObjects[0]["userName"] + ' : ' + arrOfObjects[0].score;
        let secondHighScore = document.getElementById('their-second');
        secondHighScore.innerHTML = "2-" + arrOfObjects[1]["name"] + " " + arrOfObjects[1]["userName"] + ' : ' + arrOfObjects[1].score;
        let thirdHighScore = document.getElementById('their-third');
        thirdHighScore.innerHTML = "3-" + arrOfObjects[2]["name"] + " " + arrOfObjects[2]["userName"] + ' : ' + arrOfObjects[2].score;
