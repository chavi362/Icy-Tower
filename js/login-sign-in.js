...
function cheakPassword() {
    ...
    if (pw.length < 8) {
        ...
        addMEssageAboutPassword("**Password length must contain at least one number");
    }
    else if (pw.search(/[a-zA-Z]/) == -1) {
        addMEssageAboutPassword("**Password must contain at least one English letter");
    }
    else {
...
function saveUser(e) {
    ...
    if (pwd === '' || firstName === '' || userName === '') {
        document.getElementById("submit-error-message").innerHTML = "**All the field are neccessary";
        isOk = false;
    }
    console.log(localStorage.getItem(userName))
    if (localStorage.getItem(userName) != null) {
        console.log("hffh")
        document.getElementById("submit-error-message").innerHTML = "**Sorry, there is already an acount with this email";
        return;
    }
    if (!isOk)
        return;
